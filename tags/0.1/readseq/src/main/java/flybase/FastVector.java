//flybase/FastVector.java
//split4javac// flybase/FastVector.java date=25-Aug-1999

// dclap/util/FastVector.java
// dgg - removed synchronized from Vector.java to speed up 10 fold
...
/**
 * FastVector class (a growable array).<p>
 * 
 ...
 */
public
class FastVector implements Cloneable, java.io.Serializable {
    /**
	...
	{
		java.util.Vector jv= new java.util.Vector(size());
		for (int i=0; i<size(); i++) jv.addElement( elementData[i]);
		return jv;
	}
	...
	public FastVector( java.util.Vector jv)  
	{
		this();
		if (jv!=null) 
			for (int i= 0; i<jv.size(); i++) addElement( jv.elementAt(i));
	}
	
	
		// these sub-classable
	public Object itemAt(int index) { return elementAt(index); }
	public void addItem(Object item) { addElement( item); }		
 ...
 */
//split4javac// flybase/FastVector.java line=471
