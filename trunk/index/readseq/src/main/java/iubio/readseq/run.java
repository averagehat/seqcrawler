...
{	
	...
	boolean degap, dolist, docompare, doall, dochecksum, dopipe, dolowercase, dohtml, hasoutformat,
					dotranslate, douppercase, doreverse, doPairedDocNSeq, doUnpairDocNSeq,
					dotime, doclassic, verbose, verboseClassic, doformhelp;
	char degapc= '-';
	...
	protected void initrun() 
	{
		...
		keepfeatures= nofeatlist= featlist= null;
...
//			for (int k=0; k<vals.length; k++) exfeatures.put(vals[k], "true"); 
	...
	{	
		...
		pretty.prettyInit();
		pretty.userset(); // always, so we don't drop settings somewhere?
	...
	{	
		...
		else if ( key.startsWith("compare") ) docompare= boolval;   
