...
// 07jun: these FT lines w/o nums are continuation of document of last line
//FT   DISULFID    169    183       BY SIMILARITY.
//FT                                (BY SIMILARITY).    << fails here
//FT   CHAIN        25    282       ACIDIC CHAIN (BY SIMILARITY).
//SeqRange error parsing 'BY..SIMILARITY'

//    if( level == kFeatCont || (level == kFeatField && append)) {
//		  super.addFeature( field, value, level, append);  
//      return;
//    }

    if ( level == kFeatField && !append ) {
      String start= "", stop= "";
      ...
      addFeatureNote("note",value); //  note=EXTRACELLULAR (POTENTIAL).
		  ...
		  super.addFeature( field, value, kFeatCont, append);  // kFeatCont: dont try to append location
		}
	}
...
//SQ   SEQUENCE   100 AA;  11907 MW;  5DB8D5B8 CRC32;    << swissprot/amino
	...
	{ 
		...
		putlinefield( sb, 6, getDocField(kNumN), "0", " MW; "); // "0",  " MW; "
		putlinefield( sb, 9, getDocField(kChecksum), "0", " CRC32; "); //, "0",  " CRC32; " // CRC64; also used !
		return sb.toString();  
	}
